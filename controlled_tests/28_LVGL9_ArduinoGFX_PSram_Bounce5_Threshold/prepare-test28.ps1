...
$outDir = Join-Path $repoRoot ".controlled-test-work\28_LVGL9_ArduinoGFX_PSram_Bounce5_Threshold"
$baselineFile = Join-Path $outDir "23_LVGL9_ArduinoGFX_PSramBuffers_Bounce0_VISUAL_FAIL_REFERENCE.txt"
$outFile = Join-Path $outDir "28_LVGL9_ArduinoGFX_PSram_Bounce5_Threshold.ino"
...
$dst = Replace-Once $dst `
    ...
    'static constexpr uint32_t RGB_BOUNCE_PIXELS = LCD_WIDTH * 5; // Test 28 only runtime delta: bounce = 5 lines' `
    "RGB bounce 0 to 5 lines"

$dst = $dst.Replace('23_LVGL9_ArduinoGFX_PSramBuffers_Isolation', '28_LVGL9_ArduinoGFX_PSram_Bounce5_Threshold')
$dst = $dst.Replace('23-LVGL9-GFX-PSRAM1-240901A', '28-LVGL9-GFX-PSRAM-BOUNCE5-240901A')
$dst = $dst.Replace('ESP32-8048S043 Lab / Test 23', 'ESP32-8048S043 Lab / Test 28')
$dst = $dst.Replace('double PSRAM buffers + NO RGB bounce buffer', 'double PSRAM buffers + RGB bounce 5 lines')
$dst = $dst.Replace('RGB bounce buffer = DISABLED;', 'RGB bounce buffer = 5 display lines;')
$dst = $dst.Replace('0U, static_cast<unsigned long>(RGB_BOUNCE_PIXELS)', '5U, static_cast<unsigned long>(RGB_BOUNCE_PIXELS)')
$dst = $dst.Replace('[PASS] gfx->begin() with RGB bounce buffer DISABLED', '[PASS] gfx->begin() with RGB bounce buffer requested')
$dst = $dst.Replace('2x PSRAM + NO bounce buffer', '2x PSRAM + RGB bounce 5 lines')

$required = @(
    '28-LVGL9-GFX-PSRAM-BOUNCE5-240901A',
    'static constexpr uint32_t LCD_PCLK_HZ = 14000000;',
    'static constexpr uint32_t RGB_BOUNCE_PIXELS = LCD_WIDTH * 5; // Test 28 only runtime delta: bounce = 5 lines',
    'MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT',
    'lv_display_set_color_format(display, LV_COLOR_FORMAT_RGB565);',
    'LV_DISPLAY_RENDER_MODE_PARTIAL',
    '5U, static_cast<unsigned long>(RGB_BOUNCE_PIXELS)'
)
foreach ($token in $required) { Require-Token $dst $token "generated Test 28" }

if ($dst.Contains('static constexpr uint32_t RGB_BOUNCE_PIXELS = 0;')) { throw "Test 28 still contains bounce0" }
if ($dst.Contains('MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT')) { throw "Test 28 unexpectedly moved LVGL buffers to INTERNAL SRAM" }
...
if ($inoFiles.Count -ne 1 -or $inoFiles[0].FullName -ne $outFile) {
    throw "Generated Test 28 directory must contain exactly one .ino: $outFile"
}

Write-Host ""
Write-Host "=== ESP32-8048S043 Lab / Test 28 ==="
Write-Host "FAIL baseline     : $baselineFile"
Write-Host "Generated Test 28 : $outFile"
Write-Host ""
...
Write-Host ""
Write-Host "[PASS] Test28 only runtime delta:"
Write-Host "       RGB bounce 0 -> 5 display lines"
Write-Host ""
Write-Host "Threshold matrix:"
Write-Host "       bounce0  = FAIL / touch flicker"
Write-Host "       bounce5  = physical verdict required"
Write-Host "       bounce10 = PASS"
